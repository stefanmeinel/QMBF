...
multi_exp_expE_nonsym_mat_model::multi_exp_expE_nonsym_mat_model(string _E_str, string _dE_str, string _A_str, string _B_str, string _t_str, int n_exp, int dim_1, int dim_2)
{
  E_str=_E_str;
  dE_str=_dE_str;
  A_str=_A_str;
...
int multi_exp_expE_nonsym_mat_model::get_n_functions()
{
  return dim1*dim2;
}


int multi_exp_expE_nonsym_mat_model::get_n_variables()
{
  return 1;
}


int multi_exp_expE_nonsym_mat_model::get_n_parameters()
{
  return params.size();
}


int multi_exp_expE_nonsym_mat_model::get_n_constants()
{
  return 0;
}


void multi_exp_expE_nonsym_mat_model::set_parameters(const vector< double >& parameters)
{
  params=parameters;
}


void multi_exp_expE_nonsym_mat_model::set_constants(const vector< double >& constants)
{
}


double multi_exp_expE_nonsym_mat_model::eval_function(int function, const vector< double >& arguments)
{
...
double multi_exp_expE_nonsym_mat_model::eval_derivative(int function, int parameter, const vector< double >& arguments)
{
  int v1=function/dim2;
  int v2=function%dim2;
  double t=arguments[0];
  if(v1>0)
  {
    ...
    else if( (parameter>0) && (parameter<nexp) )
    {
      ...
      sum*=(-exp(params[parameter])*t)*params[nexp+(v1-1)*nexp]*params[nexp+nexp*(dim1-1)+v2*nexp];
      return sum;
    }
  ...
  else
  {
    ...
    else if( (parameter>0) && (parameter<nexp) )
    {
      ...
      sum*=(-exp(params[parameter])*t)*params[nexp+nexp*(dim1-1)+v2*nexp];
      return sum;
    }
    else if( (parameter>=nexp) && (parameter<nexp+(dim1-1)*nexp) )
    {
...
int multi_exp_expE_nonsym_mat_model::no_of_errors()
{
  return 0;
}


string multi_exp_expE_nonsym_mat_model::get_function_name(int function, string variable)
{
  int v1=function/dim2;
  int v2=function%dim2;
  ...
  if(v1>0)
  {
    function_st << A_str << "x__" << v1+1 << "*" << A_str << "y__" << v2+1 << "*(exp(-exp(" << E_str << ")*" << variable << ")";
    for(int n=1; n<nexp; ++n)
    {
      function_st << "+" << B_str << "x_" << n << "__" << v1+1 << "*" << B_str << "y_" << n << "__" << v2+1 << "*exp(-(exp(" << E_str << ")";
      for(int k=1; k<=n; ++k)
      {
  ...
  {
    function_st << A_str << "y__" << v2+1 << "*(exp(-exp(" << E_str << ")*" << variable << ")";
    for(int n=1; n<nexp; ++n)
    {
      function_st << "+" << B_str << "y_" << n << "__" << v2+1 << "*exp(-(exp(" << E_str << ")";
      for(int k=1; k<=n; ++k)
      {
        function_st << "+exp(" << dE_str << "_" << k << ")";
      }
...
string multi_exp_expE_nonsym_mat_model::get_function_name(int function)
{
...
string multi_exp_expE_nonsym_mat_model::get_parameter_name(int parameter)
{
...
string multi_exp_expE_nonsym_mat_model::get_variable_name(int variable)
{
  if(variable!=0)
  {
    return "";
...
string multi_exp_expE_nonsym_mat_model::get_constant_name(int constant)
{
  return "";
}
