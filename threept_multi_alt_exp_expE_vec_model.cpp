...
threept_multi_alt_exp_expE_vec_model::threept_multi_alt_exp_expE_vec_model(string _E_str, string _dE_str, string _F_str, string _dF_str, string _A_str, string _B_str, string _t_str, string _T_str, int n_exp_initial, int m_exp_initial, int n_exp_final, int m_exp_final, int n_vec)
{
  E_str=_E_str;
  dE_str=_dE_str;
  F_str=_F_str;
  ...
  threept_multi_alt_exp_expE_models.resize(nvec);
  for(int v=0; v<nvec; ++v)
  {
    threept_multi_alt_exp_expE_models[v]=new threept_multi_alt_exp_expE_model(E_str, dE_str, F_str, dF_str, A_str, B_str, t_str, T_str, n_exp_initial, m_exp_initial, n_exp_final, m_exp_final);
  }
}


threept_multi_alt_exp_expE_vec_model::~threept_multi_alt_exp_expE_vec_model()
{
  for(int v=0; v<nvec; ++v)
  {
    delete threept_multi_alt_exp_expE_models[v];
  }
}


int threept_multi_alt_exp_expE_vec_model::get_n_functions()
{
  return nvec;
}


int threept_multi_alt_exp_expE_vec_model::get_n_variables()
{
  return 2;
}


int threept_multi_alt_exp_expE_vec_model::get_n_parameters()
{
  return nexp_initial+mexp_initial+nexp_final+mexp_final+nvec*(nexp_initial+mexp_initial)*(nexp_final+mexp_final);
}

int threept_multi_alt_exp_expE_vec_model::get_n_constants()
{
  return 0;
}


void threept_multi_alt_exp_expE_vec_model::set_parameters(const vector< double >& parameters)
{
  ...
  for(int v=0; v<nvec; ++v)
  {
    threept_multi_alt_exp_expE_models[v]->set_parameters(params[v]);
  }
}


void threept_multi_alt_exp_expE_vec_model::set_constants(const vector< double >& constants)
{
}


double threept_multi_alt_exp_expE_vec_model::eval_function(int function, const vector< double >& arguments)
{
  return threept_multi_alt_exp_expE_models[function]->eval_function(0, arguments);
}


double threept_multi_alt_exp_expE_vec_model::eval_derivative(int function, int parameter, const vector< double >& arguments)
{
  if( (parameter>=0) && (parameter<nexp_initial+mexp_initial+nexp_final+mexp_final) )
  {
    return threept_multi_alt_exp_expE_models[function]->eval_derivative(0, parameter, arguments);
  }
  else if( (parameter>=nexp_initial+mexp_initial+nexp_final+mexp_final) && (parameter<nexp_initial+mexp_initial+nexp_final+mexp_final+nvec*(nexp_initial+mexp_initial)*(nexp_final+mexp_final) ) )
  {
    ...
    {
      return threept_multi_alt_exp_expE_models[function]->eval_derivative(0,nexp_initial+mexp_initial+nexp_final+mexp_final+(parameter-(nexp_initial+mexp_initial+nexp_final+mexp_final))%((nexp_initial+mexp_initial)*(nexp_final+mexp_final)), arguments);
    }
    else
    {
      return 0.0;
...
int threept_multi_alt_exp_expE_vec_model::no_of_errors()
{
  return 0;
}
...
string threept_multi_alt_exp_expE_vec_model::get_single_function_name(int function, int nexpinitial, int nexpfinal, string _E_str, string _dE_str, string _F_str, string _dF_str, string _A_str, string _B_str, string variable)
{
  stringstream function_st;
  function_st << _A_str << "__" << function+1 << "*(exp(-exp(" << _F_str << ")*" << variable << ")*exp(-exp(" << _E_str << ")*(" << T_str << "-" << variable << "))";
  for(int K=0; K<nexpfinal; ++K)
  {
    for(int L=0; L<nexpinitial; ++L)
    {
      if( (L>0) || (K>0) )
      {
        function_st << "+" << _B_str << "_" << K << "_" << L << "__" << function+1 << "*exp(-(exp(" << _F_str << ")";
        for(int k=0; k<=K; ++k)
        {
          if(k>0)
          {
            function_st << "+exp(" << _dF_str << "_" << k << ")";
          }
        }
        function_st << ")*" << variable << ")*exp(-(exp(" << _E_str << ")";
        for(int l=0; l<=L; ++l)
        {
          if(l>0)
          {
            function_st << "+exp(" << _dE_str << "_" << l << ")";
          }
        }
...
string threept_multi_alt_exp_expE_vec_model::get_function_name(int function, string variable)
{
  string ee_string;
  string oe_string;
  string eo_string;
...
string threept_multi_alt_exp_expE_vec_model::get_function_name(int function)
{
  return get_function_name(function, t_str);
}


string threept_multi_alt_exp_expE_vec_model::get_parameter_name(int parameter)
{
...
string threept_multi_alt_exp_expE_vec_model::get_variable_name(int variable)
{
  if(variable==0)
  {
    return t_str;
...
string threept_multi_alt_exp_expE_vec_model::get_constant_name(int constant)
{
  return "";
}
