...
threept_multi_alt_exp_expE_model::threept_multi_alt_exp_expE_model(string _E_str, string _dE_str, string _F_str, string _dF_str, string _A_str, string _B_str, string _t_str, string _T_str, int n_exp_initial, int m_exp_initial, int n_exp_final, int m_exp_final)
{
  E_str=_E_str;
  dE_str=_dE_str;
  F_str=_F_str;
  ...
  ee_model=new threept_multi_exp_expE_model(E_str, dE_str, F_str, dF_str, A_str+"ee", B_str+"ee", t_str, T_str, nexp_initial, nexp_final);
  ee_params.resize(ee_model->get_n_parameters());

  if( mexp_final>0 )
  {
    oe_model=new threept_multi_exp_expE_model(E_str, dE_str, F_str+"o", dF_str+"o", A_str+"oe", B_str+"oe", t_str, T_str, nexp_initial, mexp_final);
    oe_params.resize(oe_model->get_n_parameters());
  }

  if( mexp_initial>0 )
  {
    eo_model=new threept_multi_exp_expE_model(E_str+"o", dE_str+"o", F_str, dF_str, A_str+"eo", B_str+"eo", t_str, T_str, mexp_initial, nexp_final);
    eo_params.resize(eo_model->get_n_parameters());
  }

  if( (mexp_initial>0) && (mexp_final>0) )
  {
    oo_model=new threept_multi_exp_expE_model(E_str+"o", dE_str+"o", F_str+"o", dF_str+"o", A_str+"oo", B_str+"oo", t_str, T_str, mexp_initial, mexp_final);
    oo_params.resize(oo_model->get_n_parameters());
  }
}


int threept_multi_alt_exp_expE_model::get_n_functions()
{
  return 1;
}


int threept_multi_alt_exp_expE_model::get_n_variables()
{
  return 2;
}


int threept_multi_alt_exp_expE_model::get_n_parameters()
{
  return nexp_initial+mexp_initial+nexp_final+mexp_final+(nexp_initial+mexp_initial)*(nexp_final+mexp_final);
}


int threept_multi_alt_exp_expE_model::get_n_constants()
{
  return 0;
}


void threept_multi_alt_exp_expE_model::set_parameters(const vector< double >& parameters)
{
...
void threept_multi_alt_exp_expE_model::set_constants(const vector< double >& constants)
{
}


double threept_multi_alt_exp_expE_model::alt(double x)
{
...
double threept_multi_alt_exp_expE_model::eval_function(int function, const vector< double >& arguments)
{
...
double threept_multi_alt_exp_expE_model::eval_derivative(int function, int parameter, const vector< double >& arguments)
{
  double t=arguments[0];
  double T=arguments[1];
  if(    (parameter>=0)
...
int threept_multi_alt_exp_expE_model::no_of_errors()
{
  return 0;
}


string threept_multi_alt_exp_expE_model::get_function_name(int function, string variable)
{
...
string threept_multi_alt_exp_expE_model::get_function_name(int function)
{
  return get_function_name(function, t_str);
}


string threept_multi_alt_exp_expE_model::get_parameter_name(int parameter)
{
...
string threept_multi_alt_exp_expE_model::get_variable_name(int variable)
{
  return ee_model->get_variable_name(variable);
}


string threept_multi_alt_exp_expE_model::get_constant_name(int constant)
{
