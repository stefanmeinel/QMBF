...
multi_exp_mat_upper_model::multi_exp_mat_upper_model(string _E_str, string _dE_str, string _A_str, string _B_str, string _t_str, int n_exp, int _dim)
{
  E_str=_E_str;
  dE_str=_dE_str;
  A_str=_A_str;
...
int multi_exp_mat_upper_model::get_n_functions()
{
  return dim*(dim+1)/2;
}


int multi_exp_mat_upper_model::get_n_variables()
{
  return 1;
}


int multi_exp_mat_upper_model::get_n_parameters()
{
  return nexp+dim*nexp;
}

int multi_exp_mat_upper_model::get_n_constants()
{
  return 0;
}


void multi_exp_mat_upper_model::set_parameters(const vector< double >& parameters)
{
  params=parameters;
}


void multi_exp_mat_upper_model::set_constants(const vector< double >& constants)
{
}


double multi_exp_mat_upper_model::eval_function(int function, const vector< double >& arguments)
{
...
double multi_exp_mat_upper_model::eval_derivative(int function, int parameter, const vector< double >& arguments)
{
  int v1=f_to_v1[function];
  int v2=f_to_v2[function];
  ...
  else if( (parameter>0) && (parameter<nexp) )
  {
    ...
    sum*=(-t)*params[nexp+v1*nexp]*params[nexp+v2*nexp];
    return sum;
  }
...
int multi_exp_mat_upper_model::no_of_errors()
{
  return 0;
}


string multi_exp_mat_upper_model::get_function_name(int function, string variable)
{
  int v1=f_to_v1[function];
  int v2=f_to_v2[function];
  stringstream function_st;
  function_st << A_str << "__" << v1+1 << "*" << A_str << "__" << v2+1 << "*(exp(-" << E_str << "*" << variable << ")";
  for(int n=1; n<nexp; ++n)
  {
    function_st << "+" << B_str << "_" << n << "__" << v1+1 << "*" << B_str << "_" << n << "__" << v2+1 << "*exp(-(" << E_str;
    for(int k=1; k<=n; ++k)
    {
...
string multi_exp_mat_upper_model::get_function_name(int function)
{
  return get_function_name(function, t_str);
}


string multi_exp_mat_upper_model::get_parameter_name(int parameter)
{
...
string multi_exp_mat_upper_model::get_variable_name(int variable)
{
  if(variable!=0)
  {
    return "";
...
string multi_exp_mat_upper_model::get_constant_name(int constant)
{
  return "";
}
