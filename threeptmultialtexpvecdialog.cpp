...
threept_multi_alt_exp_vec_dialog::threept_multi_alt_exp_vec_dialog(int n_exp_initial, int max_n_exp_initial, int m_exp_initial, int max_m_exp_initial, int n_exp_final, int max_n_exp_final, int m_exp_final, int max_m_exp_final, int n_vec, int max_n_vec, QWidget* parent)
{
  ...
  setWindowTitle(tr("Create 3-Point Multiple (incl. ocsillating) Exponentials Vector Fit Model"));

  createGadgets();
}

void threept_multi_alt_exp_vec_dialog::set_text(QString text)
{
  text_LB->setText(text);
}


int threept_multi_alt_exp_vec_dialog::get_n_exp_initial()
{
  return n_exp_initial_SB->value();
}

void threept_multi_alt_exp_vec_dialog::set_n_exp_initial(int val)
{
  n_exp_initial_SB->setValue(val);
}

int threept_multi_alt_exp_vec_dialog::get_m_exp_initial()
{
  return m_exp_initial_SB->value();
}

void threept_multi_alt_exp_vec_dialog::set_m_exp_initial(int val)
{
  m_exp_initial_SB->setValue(val);
}


int threept_multi_alt_exp_vec_dialog::get_n_exp_final()
{
  return n_exp_final_SB->value();
}

void threept_multi_alt_exp_vec_dialog::set_n_exp_final(int val)
{
  n_exp_final_SB->setValue(val);
}


int threept_multi_alt_exp_vec_dialog::get_m_exp_final()
{
  return m_exp_final_SB->value();
}

void threept_multi_alt_exp_vec_dialog::set_m_exp_final(int val)
{
  m_exp_final_SB->setValue(val);
}


int threept_multi_alt_exp_vec_dialog::get_n_vec()
{
  return n_vec_SB->value();
}

void threept_multi_alt_exp_vec_dialog::set_n_vec(int val)
{
  n_vec_SB->setValue(val);
}

void threept_multi_alt_exp_vec_dialog::reset()
{
...
void threept_multi_alt_exp_vec_dialog::modified_slot()
{
  emit modified();
}


void threept_multi_alt_exp_vec_dialog::createGadgets()
{
  ...
  text_LB=new QLabel("3-Point Correlator, Multiple (incl. oscillating) Exponentials, Vector Fit");
  ...
  layout->addWidget(m_exp_initial_LB, 2, 0);
  layout->addWidget(m_exp_initial_SB, 2, 1);

  layout->addWidget(n_exp_final_LB, 3, 0);
  layout->addWidget(n_exp_final_SB, 3, 1);

  layout->addWidget(m_exp_final_LB, 4, 0);
  layout->addWidget(m_exp_final_SB, 4, 1);
